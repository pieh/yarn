...
export function filterOverridenGitignores(files: WalkFiles, joinFn = path.join): WalkFiles {
  ...
  return files.filter(file => IGNORE_FILENAMES.indexOf(file.basename) > -1).reduce((acc: WalkFiles, file) => {
    if (file.basename !== GITIGNORE_NAME) {
      ...
      const higherPriorityIgnoreFilePaths = [joinFn(dir, IGNORE_FILENAMES[0]), joinFn(dir, IGNORE_FILENAMES[1])];
